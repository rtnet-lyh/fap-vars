...
- `server/`: Rocky, Windows, Solaris, HP-UX, ESXi 같은 서버 OS 계열 자료를 둔다.
...
새 원천 케이스는 `raw_data/<domain>/<platform_or_product>/...` 아래에 배치한다. 예: `server/rocky/rocky_memory_usage_free_check.md`, `network/nx_os/mds_c9148s/1_1_cpu.md`, `was/jeus/rocky/was_1_1_ps_cpu_chk.md`, `backup/veritas/netbackup_appliance_5240/1_1_catalog.md`.
...
관련 없는 working tree 변경을 되돌리지 않는다. `server/rocky/`에는 기존 `linux_*_sample.md` 이름에서 새 `rocky_*_check.md` 이름으로 바꾸는 작업이 진행 중일 수 있다. 다른 도메인은 같은 제품 또는 장비 계열의 기존 Markdown schema와 파일명 패턴을 우선 따른다. 명시 요청이 없는 한 runtime 또는 replay bundle 파일을 새로 만들지 않는다.
