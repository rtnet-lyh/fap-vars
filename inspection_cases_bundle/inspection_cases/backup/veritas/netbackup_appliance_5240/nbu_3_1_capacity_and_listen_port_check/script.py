...
class Check(BaseCheck):
    ...
    def run(self):
        ...
        if usage_percent > max_usage_percent or denied_found:
            return self.warn(metrics=metrics, thresholds=thresholds, reasons='Use%가 기준을 초과했거나 금지 포트가 LISTEN 상태입니다.', message='용량/포트 상태 경고: Use%%=%s, 금지 포트=%s.' % (usage_percent, ','.join(denied_found) or '없음'))
