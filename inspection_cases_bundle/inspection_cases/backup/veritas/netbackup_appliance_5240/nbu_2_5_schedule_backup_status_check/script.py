...
class Check(BaseCheck):
    ...
    def run(self):
        ...
        invalid_rows = [row for row in scheduled_rows if row['state'] != 'Done' or row['status_code'] != '0']
        metrics = {
            'job_count': len(rows),
        ...
        if invalid_rows:
            return self.warn(metrics=metrics, thresholds=thresholds, reasons='스케줄이 있는 작업 중 Done/0 기준을 만족하지 않는 행이 있습니다.', message='스케줄 백업 상태 경고: 비정상 스케줄 작업 %s건.' % len(invalid_rows))
