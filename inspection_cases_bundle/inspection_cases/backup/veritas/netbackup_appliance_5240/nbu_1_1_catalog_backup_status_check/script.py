...
class Check(BaseCheck):
    ...
    def run(self):
        ...
        if invalid_rows:
            return self.warn(metrics=metrics, thresholds=thresholds, reasons='Catalog Backup 작업 중 Done/0 기준을 만족하지 않는 행이 있습니다.', message='카탈로그 백업 상태 경고: 비정상 작업 %s건.' % len(invalid_rows))
        return self.ok(metrics=metrics, thresholds=thresholds, reasons='모든 Catalog Backup 작업의 State가 Done이고 Statu 값이 0입니다.', message='카탈로그 백업 상태 점검 정상')
