...
- Windows 계열은 `CONNECTION_METHOD = 'winrm'` 또는 실행 계정 형식 `winrm`와 `_run_ps("...")` 패턴을 사용한다.
- Paramiko 옵션은 API credential `data`에 넣지 말고 `host_vars`의 `PARAMIKO_*` 값을 우선 사용한다. 케이스 공통 기본값이 필요할 때만 `script.py`의 `PARAMIKO_*` 클래스 속성으로 조정한다.
...
- `matcher_value`는 `script.py`의 실제 `_run_paramiko_commands(...)`, 기존 `_ssh(...)`, `_run_ps(...)` 명령과 완전히 같아야 한다.
