...
CHECK_KIND = 'cpu_usage'
ITEM_NAME = 'CPU 사용률'
PS_COMMAND = "$ErrorActionPreference = 'Stop'; $sample = Get-Counter '\\Processor(_Total)\\% Processor Time'; $value = [math]::Round($sample.CounterSamples[0].CookedValue, 2); [pscustomobject]@{ cpu_usage_percent = $value } | ConvertTo-Json -Compress -Depth 6"
MANUAL_MESSAGE = ''
