...
CHECK_KIND = 'disk_redundancy'
ITEM_NAME = 'Disk 이중화 정상 여부'
PS_COMMAND = "$ErrorActionPreference = 'Stop'; $physical = @(); $virtual = @(); $storageModuleAvailable = $false; if (Get-Command Get-PhysicalDisk -ErrorAction SilentlyContinue) { $storageModuleAvailable = $true; $physical = @(Get-PhysicalDisk | Select-Object FriendlyName, HealthStatus, OperationalStatus, MediaType, Size) }; if (Get-Command Get-VirtualDisk -ErrorAction SilentlyContinue) { $storageModuleAvailable = $true; $virtual = @(Get-VirtualDisk | Select-Object FriendlyName, HealthStatus, OperationalStatus, ResiliencySettingName, Size) }; [pscustomobject]@{ storage_module_available = $storageModuleAvailable; physical_disks = $physical; virtual_disks = $virtual } | ConvertTo-Json -Compress -Depth 6"
MANUAL_MESSAGE = ''
