...
CHECK_KIND = 'network_link'
ITEM_NAME = 'NW 링크 상태 연결속도 설정'
PS_COMMAND = "$ErrorActionPreference = 'Stop'; $adapters = Get-NetAdapter | Select-Object Name, InterfaceDescription, Status, LinkSpeed, MacAddress; $down = @($adapters | Where-Object { $_.Status -ne 'Up' }); [pscustomobject]@{ adapter_count = @($adapters).Count; down_adapter_count = @($down).Count; adapters = $adapters } | ConvertTo-Json -Compress -Depth 6"
MANUAL_MESSAGE = ''
