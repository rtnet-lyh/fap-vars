...
CHECK_KIND = 'event_cpu'
ITEM_NAME = 'CPU 로그'
PS_COMMAND = "$ErrorActionPreference = 'Stop'; $since = (Get-Date).AddHours(-24); $events = @(Get-WinEvent -FilterHashtable @{LogName='System'; Level=1,2,3; StartTime=$since} -ErrorAction SilentlyContinue | Where-Object { ($_.ProviderName + ' ' + $_.Message) -match 'processor|cpu|thermal|machine check' } | Select-Object -First 20 TimeCreated, Id, LevelDisplayName, ProviderName, Message); [pscustomobject]@{ lookback_hours = 24; keyword_pattern = 'processor|cpu|thermal|machine check'; event_count = @($events).Count; events = $events } | ConvertTo-Json -Compress -Depth 6"
MANUAL_MESSAGE = ''
