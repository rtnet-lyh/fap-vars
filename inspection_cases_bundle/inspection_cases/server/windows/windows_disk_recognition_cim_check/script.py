...
CHECK_KIND = 'disk_recognition'
ITEM_NAME = 'Disk 인식 여부 점검'
PS_COMMAND = "$ErrorActionPreference = 'Stop'; $disks = Get-CimInstance Win32_DiskDrive | Select-Object DeviceID, Model, InterfaceType, MediaType, Size, Status; $bad = @($disks | Where-Object { $_.Status -and $_.Status -ne 'OK' }); [pscustomobject]@{ disk_count = @($disks).Count; abnormal_disk_count = @($bad).Count; disks = $disks } | ConvertTo-Json -Compress -Depth 6"
MANUAL_MESSAGE = ''
