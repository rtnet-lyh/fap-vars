...
CHECK_KIND = 'disk_swap'
ITEM_NAME = 'Disk Swap 사용률'
PS_COMMAND = "$ErrorActionPreference = 'Stop'; $page = Get-CimInstance Win32_PageFileUsage | Select-Object Name, AllocatedBaseSize, CurrentUsage, PeakUsage; $allocated = ($page | Measure-Object -Property AllocatedBaseSize -Sum).Sum; $current = ($page | Measure-Object -Property CurrentUsage -Sum).Sum; $usage = if ($allocated -gt 0) { [math]::Round(($current / $allocated) * 100, 2) } else { 0 }; [pscustomobject]@{ pagefile_count = @($page).Count; allocated_mb = [int64]$allocated; current_usage_mb = [int64]$current; pagefile_usage_percent = $usage; pagefiles = $page } | ConvertTo-Json -Compress -Depth 6"
MANUAL_MESSAGE = ''
