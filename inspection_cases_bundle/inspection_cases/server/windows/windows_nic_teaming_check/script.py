...
CHECK_KIND = 'nic_teaming'
ITEM_NAME = 'NIC 이중화 점검'
PS_COMMAND = "$ErrorActionPreference = 'Stop'; $available = [bool](Get-Command Get-NetLbfoTeam -ErrorAction SilentlyContinue); $teams = @(); if ($available) { $teams = @(Get-NetLbfoTeam | Select-Object Name, Status, TeamingMode, LoadBalancingAlgorithm) }; [pscustomobject]@{ get_net_lbfo_team_available = $available; team_count = @($teams).Count; teams = $teams } | ConvertTo-Json -Compress -Depth 6"
MANUAL_MESSAGE = ''
