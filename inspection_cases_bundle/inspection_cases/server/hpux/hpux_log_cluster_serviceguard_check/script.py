...
DEFAULT_BAD_LOG_KEYWORDS = r'status\s+change.*offline|status\s+change.*unknown|cluster\s+error|communication\s+failure|\berror\b'
DEFAULT_IGNORE_LOG_KEYWORDS = r'status\s+change.*online'
REGEX_FLAGS = re.IGNORECASE
FAIL_ERROR = '클러스터 장애 로그 감지'
LOG_LABEL = '클러스터 장애 로그'
ITEM_LABEL = '클러스터 로그'
