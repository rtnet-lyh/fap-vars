...
class Check(BaseCheck):
    ...
    def execute_check(self):
        ...
        degraded = [
            ...
            if entry.get('HealthStatus', '').lower() not in ('healthy', 'ok')
            or (
                'ok' not in entry.get('OperationalStatus', 'ok').lower()
                and 'completed' not in entry.get('OperationalStatus', 'ok').lower()
            )
        ]

        spare_count = sum(1 for entry in entries if entry.get('Resiliency', '').lower() == 'spare')
        ...
        if require_spare_device and spare_count < require_spare_device:
            return self.warn(
                ...
                reasons='예비 디스크 수가 기준치 미만입니다.',
                message=f'Windows 디스크 HA 점검 결과 예비 디스크 수가 기준치 미만입니다. 현재 상태: 가상 디스크 {virtual_disk_count}개, spare {spare_count}개 (기준 {require_spare_device}개 이상).',
            )
        ...
        return self.ok(
            ...
            thresholds={
                '예비 장치 필요 여부': require_spare_device,
                '실패 키워드': failure_keywords,
            },
            reasons=f'가상 디스크 {virtual_disk_count}개, spare {spare_count}개 (기준 {require_spare_device}개 이상).',
            message='가상 디스크 HA 상태가 기준 범위 내입니다.',
        )
