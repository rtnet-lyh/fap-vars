...
SWAP_MEMORY_COMMAND = "wmic pagefile get Name,AllocatedBaseSize,CurrentUsage,PeakUsage /value"
...
class Check(BaseCheck):
    ...
    def execute_check(self):
        ...
        for page in pages:
            try:
                allocated = float(page.get('AllocatedBaseSize', '0') or '0')
                current_usage = float(page.get('CurrentUsage', '0') or '0')
                peak = float(page.get('PeakUsage', '0') or '0')
            except ValueError:
                continue
            usage_percent = round((current_usage / allocated) * 100, 2) if allocated > 0 else 0.0
            parsed.append({
                'name': page.get('Name', ''),
                'allocated_gib': round(allocated / 1024, 2),
                'current_usage_gib': round(current_usage / 1024, 2),
                'peak_usage_gib': round(peak / 1024, 2),
        ...
        max_entry = max(parsed, key=lambda item: item['usage_percent'])
        if max_entry['usage_percent'] >= max_swap_usage_percent:
            return self.fail('스왑 사용률 임계치 초과', message='스왑 사용률이 기준치를 초과한 페이지 파일이 확인되었습니다.', stdout=text, stderr=(err or '').strip())

        return self.ok(
            metrics={
                'pagefile_count': len(parsed),
                'pagefiles': parsed,
            ...
            reasons=f'Windows 스왑 메모리 사용률 점검이 정상입니다. 현재 상태: 페이지 파일 {len(parsed)}개, 최고 사용률 {max_entry["usage_percent"]:.2f}% (기준 {max_swap_usage_percent:.2f}% 미만).',
            message='모든 페이지 파일 사용률이 기준 범위 내입니다.',
        )
