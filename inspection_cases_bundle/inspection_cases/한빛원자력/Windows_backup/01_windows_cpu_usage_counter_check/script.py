...
class Check(BaseCheck):
    ...
    def execute_check(self):
        ...
        if matched_failure_keywords:
            return self.fail(
                'CPU 점검 실패 키워드 감지',
                message='CPU 사용률 결과에서 실패 키워드가 확인되었습니다.',
                stdout=text,
                stderr=(err or '').strip()
            )
        ...
        if not {'user', 'privileged', 'idle', 'interrupt'}.issubset(values):
            return self.fail(
                'CPU 통계 파싱 실패',
                message='CPU 사용률 key=value 결과를 해석하지 못했습니다.',
                stdout=text,
                stderr=(err or '').strip()
            )
        ...
        return self.ok(
            ...
            thresholds={
                '사용자+시스템 CPU 사용률 최대 기준': max_usr_sys_percent\n
                'CPU 유휴율 최소 기준': min_idle_percent\n
                '인터럽트 처리 비율 최대 기준': max_interrupt_percent\n
                '실패 키워드': failure_keywords
            },
            reasons=(
                f'사용자 CPU 사용률 {avg_user_percent:.2f}%, '
                f'시스템 CPU 사용률 {avg_privileged_percent:.2f}%, '
                f'사용자+시스템 CPU 사용률 {avg_usr_sys_percent:.2f}% '
                f'(기준 {max_usr_sys_percent:.2f}% 이하), '
                f'CPU 유휴율 {avg_idle_percent:.2f}% '
                f'(기준 {min_idle_percent:.2f}% 이상), '
                f'인터럽트 처리 비율 {avg_interrupt_percent:.2f}% '
