...
- 계산 가능한 NTFS 볼륨만 대상으로 하며 `IUsed(approx)` 또는 `IUse%(approx)`를 계산할 수 없으면 불량으로 처리합니다.

# 임계치
- `max_iuse_percent`: `80.0`
  - 현재 스크립트에서 threshold로 노출되는 호환용 값입니다.
  - Windows 점검에서는 inode 사용률 대신 볼륨 Dirty 상태를 판정합니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 볼륨 상태 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.

# 판단기준
- **정상**: 모든 측정 가능 볼륨의 `IUse%`가 임계치 이하입니다.
- **불량**: inode 유사 사용률 계산이 불가능하거나, 하나 이상의 볼륨에서 inode 유사 사용률이 임계치를 초과합니다.
