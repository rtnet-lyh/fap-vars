...
- `max_critical_error_count`: `0`
  - Critical/Error 이벤트 수가 `0`건 이하이면 정상입니다.
  - Critical/Error 이벤트 수가 `0`건 초과이면 기준 미충족입니다.
- `max_warning_count`: `10`
  - Warning 이벤트 수가 `10`건 이하이면 정상입니다.
  - Warning 이벤트 수가 `10`건 초과이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 시스템 로그 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
