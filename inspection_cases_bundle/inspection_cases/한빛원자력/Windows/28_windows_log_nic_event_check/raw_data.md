...
# 임계치
- `min_up_nic_count`: `1`
  - Up 상태 서비스 NIC 수가 `1`개 이상이면 정상입니다.
  - Up 상태 서비스 NIC 수가 `1`개 미만이면 기준 미충족입니다.
- `max_nic_event_count`: `0`
  - NIC 부정 이벤트 수가 `0`건 이하이면 정상입니다.
  - NIC 부정 이벤트 수가 `0`건 초과이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 NIC 로그 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
