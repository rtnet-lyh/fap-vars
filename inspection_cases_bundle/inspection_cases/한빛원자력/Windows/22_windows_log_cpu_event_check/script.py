...
CHECK_COMMAND = "$e=Get-WinEvent -FilterHashtable @{LogName='System';StartTime=(Get-Date).AddDays(-30);Level=@(1,2,3)} -ErrorAction SilentlyContinue | Where-Object { $_.ProviderName -in @('Microsoft-Windows-WHEA-Logger','Microsoft-Windows-Kernel-Processor-Power') -or $_.Message -match '(?i)\\bECC\\b|uncorrectable|processor|cpu|offline' }; if($e){@($e | Select-Object TimeCreated,ProviderName,Id,LevelDisplayName,@{N='Message';E={($_.Message -replace '\\r?\\n',' ')}}) | ConvertTo-Json -Depth 4}else{'No CPU/ECC/offline-like events found in the last 30 days.'}"
CASE_ID = '22_windows_log_cpu_event_check'
CHECK_NAME = 'Windows CPU 로그'
THRESHOLD_DEFS = [('max_cpu_ecc_event_count', 0, 'int'), ('failure_keywords', '', 'str')]
