...
# 임계치
- `min_socket_count`: `1`
  - 물리 CPU 소켓 수가 `1`개 이상이면 정상입니다.
  - 물리 CPU 소켓 수가 `1`개 미만이면 기준 미충족입니다.
- `min_total_core_count`: `4`
  - 전체 물리 CPU 코어 수가 `4`개 이상이면 정상입니다.
  - 전체 물리 CPU 코어 수가 `4`개 미만이면 기준 미충족입니다.
- `min_total_logical_processor_count`: `8`
  - 전체 논리 CPU 수가 `8`개 이상이면 정상입니다.
  - 전체 논리 CPU 수가 `8`개 미만이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 CPU 코어 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
