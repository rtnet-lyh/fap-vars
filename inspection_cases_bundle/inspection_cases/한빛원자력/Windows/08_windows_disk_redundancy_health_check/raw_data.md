...
- `require_spare_device`: `0`
  - 값이 `0`이면 예비 디스크 수 기준을 적용하지 않습니다.
  - 값이 `1` 이상이면 spare 디스크 수가 해당 값 이상이어야 정상입니다.
  - spare 디스크 수가 설정값 미만이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 디스크 HA 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
