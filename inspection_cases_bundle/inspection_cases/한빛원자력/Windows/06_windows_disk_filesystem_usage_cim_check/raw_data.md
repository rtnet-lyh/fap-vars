...
- `max_usage_percent`: `80.0`
  - 모든 볼륨의 사용률이 `80.0%` 미만이면 정상입니다.
  - 하나 이상의 볼륨 사용률이 `80.0%` 이상이면 기준 미충족입니다.
- `min_available_percent`: `20.0`
  - 모든 볼륨의 가용률이 `20.0%` 초과이면 정상입니다.
  - 하나 이상의 볼륨 가용률이 `20.0%` 이하이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 디스크 사용률 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
