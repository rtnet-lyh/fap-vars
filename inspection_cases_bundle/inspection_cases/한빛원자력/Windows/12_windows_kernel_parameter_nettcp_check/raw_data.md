...
- `expected_ip_forward`: `0`
  - `IPEnableRouter` 값이 `0`이면 정상입니다.
  - `IPEnableRouter` 값이 `0`이 아니면 기준 미충족입니다.
- `disallowed_accept_source_route_values`: `0`
  - Source Routing Behavior 값이 이 목록에 포함되지 않으면 정상입니다.
  - Source Routing Behavior 값이 이 목록에 포함되면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 커널 파라미터 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
