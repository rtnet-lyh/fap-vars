...
# 임계치
- `min_disk_count`: `1`
  - 인식된 디스크 수가 `1`개 이상이면 정상입니다.
  - 인식된 디스크 수가 `1`개 미만이면 기준 미충족입니다.
- `min_partition_count`: `1`
  - 인식된 파티션 수가 `1`개 이상이면 정상입니다.
  - 인식된 파티션 수가 `1`개 미만이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 디스크 인식 상태 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
