...
CHECK_COMMAND = "if (Get-Command Get-MPIOSetting -ErrorAction SilentlyContinue) { Get-MPIOSetting | Select-Object PathVerificationState,PathVerificationPeriod,RetryCount,RetryInterval,DiskTimeoutValue,@{N='LoadBalancePolicy';E={Get-MSDSMGlobalDefaultLoadBalancePolicy 2>$null}} | Format-List; mpclaim.exe -s -d } else { 'MPIO 미설치 또는 미지원' }"
CASE_ID = '19_windows_os_mpio_path_check'
CHECK_NAME = 'Windows MPIO 경로 이중화'
THRESHOLD_DEFS = [('expected_policy_keyword', 'round', 'str'), ('failure_keywords', '', 'str')]
