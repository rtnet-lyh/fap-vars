...
- `max_down_node_count`: `0`
  - Down 노드 수가 `0`개 이하이면 정상입니다.
  - Down 노드 수가 `0`개 초과이면 기준 미충족입니다.
- `max_offline_resource_count`: `0`
  - Offline 리소스 수가 `0`개 이하이면 정상입니다.
  - Offline 리소스 수가 `0`개 초과이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 클러스터 상태 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
