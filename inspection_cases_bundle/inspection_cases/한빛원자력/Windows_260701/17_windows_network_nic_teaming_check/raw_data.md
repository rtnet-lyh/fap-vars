...
- `max_down_or_degraded_team_count`: `0`
  - Down/Degraded 팀 수가 `0`개 이하이면 정상입니다.
  - Down/Degraded 팀 수가 `0`개 초과이면 기준 미충족입니다.
- `max_failed_member_count`: `0`
  - 비정상 멤버 수가 `0`개 이하이면 정상입니다.
  - 비정상 멤버 수가 `0`개 초과이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 NIC Teaming 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.
