...
class Check(BaseCheck):
    ...
    def execute_check(self):
        ...
        return self.ok(
            ...
            thresholds={
                '최대 비정상 팀 수': max_down_or_degraded_team_count,
                '최대 실패 멤버 수': max_failed_member_count,
                '실패 키워드': failure_keywords,
            },
            reasons=f'팀 {len(entries)}개, Down/Degraded 팀 {len(down_or_degraded_teams)}개, 멤버 수 미달 팀 {len(failed_members)}개.',
