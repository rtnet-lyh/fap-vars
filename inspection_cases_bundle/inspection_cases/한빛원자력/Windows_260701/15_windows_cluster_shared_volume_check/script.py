...
CHECK_COMMAND = '$p=\'C:\\mnt\\shared\\\'; $pt=Get-Partition | Where-Object { $_.AccessPaths -contains $p -or $_.AccessPaths -contains $p.TrimEnd(\'\\\') }; if($pt){ $v=$pt | Get-Volume; [pscustomobject]@{Device="Disk$($pt.DiskNumber)\\Partition$($pt.PartitionNumber)"; MountedOn=$p.TrimEnd(\'\\\'); FileSystem=$v.FileSystem; Mode=$(if($pt.IsReadOnly){\'ro\'}else{\'rw\'}); Status=$pt.OperationalStatus; Health=$v.HealthStatus} | Format-List } else { "Mount point not found: $p" }'
CASE_ID = '15_windows_cluster_shared_volume_check'
CHECK_NAME = 'Windows 클러스터 공유 볼륨'
THRESHOLD_DEFS = [('expected_mount_path', 'C:\\mnt\\shared\\', 'str'), ('expected_mode', 'rw', 'str'), ('failure_keywords', '', 'str')]
