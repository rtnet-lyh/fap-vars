...
- `max_usr_sys_percent`: `80.0`
  - 평균 사용자+시스템 CPU 사용률이 `80.0%` 미만이면 정상입니다.
  - 평균 사용자+시스템 CPU 사용률이 `80.0%` 이상이면 기준 미충족입니다.
- `min_idle_percent`: `20.0`
  - 평균 Idle 비율이 `20.0%` 이상이면 정상입니다.
  - 평균 Idle 비율이 `20.0%` 미만이면 기준 미충족입니다.
- `max_interrupt_percent`: `5.0`
  - 평균 Interrupt Time이 `5.0%` 미만이면 정상입니다.
  - 평균 Interrupt Time이 `5.0%` 이상이면 기준 미충족입니다.
- `failure_keywords`: 없음
  - 실패 키워드가 설정되면 typeperf 원문 출력에 해당 키워드가 포함될 때 기준 미충족입니다.

# 판단기준
- **정상**: 평균 `usr+sys`가 `80.0%` 미만이고, 평균 idle 비율이 `20.0%` 이상이며, 평균 interrupt 비율이 `5.0%` 미만입니다.
- **실패**: 평균 `usr+sys`가 `80.0%` 이상이거나, 평균 idle 비율이 `20.0%` 미만이거나, 평균 interrupt 비율이 `5.0%` 이상입니다.
