...
CHECK_COMMAND = "Get-CimInstance Win32_PageFileUsage | Select-Object @{N='Filename';E={$_.Name}},@{N='Type';E={'file'}},@{N='Size(MB)';E={$_.AllocatedBaseSize}},@{N='Used(MB)';E={$_.CurrentUsage}},@{N='Usage(%)';E={if($_.AllocatedBaseSize){[math]::Round(($_.CurrentUsage/$_.AllocatedBaseSize)*100,2)}else{0}}},@{N='Peak(MB)';E={$_.PeakUsage}}"
CASE_ID = '05_windows_memory_pagefile_usage_cim_check'
CHECK_NAME = 'Windows PageFile 사용률'
THRESHOLD_DEFS = [('max_swap_usage_percent', 50.0, 'float'), ('failure_keywords', '', 'str')]
