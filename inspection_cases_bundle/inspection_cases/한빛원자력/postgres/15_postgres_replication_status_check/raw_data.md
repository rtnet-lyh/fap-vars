...
# 점검 내용
PostgreSQL 이중화/복제 상태를 Windows 환경의 DB 클라이언트 명령으로 점검합니다.
...
# 명령어
```powershell
$OutputEncoding = [System.Text.UTF8Encoding]::new($false); [Console]::OutputEncoding = [System.Text.UTF8Encoding]::new($false); psql -Atqc "SELECT application_name, state, sync_state, COALESCE(write_lag::text,'0'), COALESCE(replay_lag::text,'0') FROM pg_stat_replication;" postgres
```
