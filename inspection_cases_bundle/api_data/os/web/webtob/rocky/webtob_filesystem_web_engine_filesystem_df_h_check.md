...
WEB 엔진 설치 파일시스템 FULL로 인한 서비스 불가 확인을 위한 파일시스템 점검

# inspection_command

```bash
- web_engine_fs_path 변수
```bash
df -h "{{ web_engine_fs_path }}" # web_engine_fs_path: /home/exTMS/tmax/webtob
```
```
...
```text
[root@sd_tipswebwas webtob]# df -h /home/exTMS/tmax/webtob
...
[
    {id: null, key: "web_engine_fs_path", value: "/home/exTMS/tmax/webtob", sortOrder: 0}
,
...
class Check(BaseCheck):
    ...
    DEFAULT_PATH = '/home/exTMS/tmax/webtob'
    DEFAULT_MAX_USE_PERCENT = 80
    COMMAND_TIMEOUT = 10

    def _target_path(self):
        host_path = str(self.get_host_var('web_engine_fs_path', '') or '').strip()
        if host_path:
            return host_path, 'host_vars'

        threshold_path = str(
            self.get_threshold_var('web_engine_fs_path', default='', value_type='str') or ''
        ).strip()
        if threshold_path:
    ...
    def run(self):
        ...
        if result.get('rc') != 0:
            return self.fail(
                'df 명령 실행 실패',
                message='WEB 엔진 파일시스템 사용률을 확인하지 못했습니다.',
                stdout=stdout,
                stderr=stderr,
            )
        ...
        metrics = {
            'web_engine_fs_path': target_path,
            'web_engine_fs_path_source': path_source,
        ...
        if parsed['use_percent'] > max_use_percent:
            return self.warn(
                ...
                message='WEB 엔진 파일시스템 사용률 경고: %s 사용률 %s%%, 기준 %s%%' % (
                    target_path,
                    parsed['use_percent'],
                    max_use_percent,
                ),
        ...
        return self.ok(
            ...
            message='WEB 엔진 파일시스템 사용률 정상: %s 사용률 %s%%, 기준 %s%%' % (
                target_path,
                parsed['use_percent'],
                max_use_percent,
            ),
