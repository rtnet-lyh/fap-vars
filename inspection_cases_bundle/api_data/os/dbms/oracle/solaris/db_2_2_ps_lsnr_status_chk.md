...
class Check(BaseCheck):
    ...
    def run(self):
        ...
        if result.get('rc') not in (0, 1):
            return self.fail('ps 명령 실행 실패', message='리스너 점검 문서의 ora_smon 프로세스 명령을 실행하지 못했습니다.', stdout=stdout, stderr=stderr)
        ...
        if not lines:
            return self.fail(
                ...
                message='리스너 프로세스 점검 raw 문서의 ps 출력에서 ora_smon 프로세스를 찾지 못했습니다.',
                stdout=stdout,
                stderr=stderr,
            )
        return self.ok(
            metrics=metrics,
            thresholds=thresholds,
            reasons='raw 문서 명령 기준 ora_smon 프로세스가 확인되었습니다.',
            message='리스너 프로세스 점검 raw 명령 결과 정상: ora_smon %s건' % len(lines),
        )
