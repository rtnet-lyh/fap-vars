...
[
    {id: null, key: "bad_log_keywords", value: "failed,not detected,error,fault,failure", sortOrder: 0}
,
...
LOG_COMMAND = "dmesg | egrep -i 'psu|power supply|failed'"
...
class Check(BaseCheck):
    ...
    def run(self):
        bad_log_keywords = self._split_keywords(self.get_threshold_var('bad_log_keywords', default='failed,not detected,error,fault,failure', value_type='str'))
        ...
        result = self._run_solaris_commands([
            {'command': LOG_COMMAND, 'timeout': 15},
        ])[0]
        rc = result['rc']
        out = result['stdout']
        err = result['stderr']
        ...
        if rc not in (0, 1):
            return self.fail(
                '점검 명령 실행 실패',
                message=(
                    'Solaris POWER 로그 점검에 실패했습니다. '
                    "현재 상태: dmesg | grep -i 'psu|power supply|failed' 명령을 정상적으로 실행하지 못했습니다."
                ),
                stdout=(out or '').strip(),
                stderr=(err or '').strip(),
            )
        ...
        if bad_lines:
            return self.fail(
                ...
                message=(
                    'Solaris POWER 로그 점검에 실패했습니다. '
                    f'현재 상태: POWER 관련 로그 {len(lines)}건 중 비정상 로그 {len(bad_lines)}건이 확인되었습니다. '
        ...
        return self.ok(
            ...
            reasons='전원공급장치 관련 비정상 로그가 검출되지 않았고 stderr도 없습니다.',
            message=(
                f'Solaris POWER 로그가 정상입니다. 현재 상태: POWER 관련 로그 {len(lines)}건 조회, '
