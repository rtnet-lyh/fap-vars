...
class Check(BaseCheck):
    ...
    def _run_command(self):       
        ...
        if result.get('rc') != 0:
            return None, self.fail('점검 명령 실행 실패', message='스케줄 백업 상태 조회 명령 실행에 실패했습니다.', stdout=stdout, stderr=stderr)
        return stdout, None
    ...
    def _parse_jobs(self, stdout):
        ...
        for line in str(stdout or '').splitlines():
            ...
            if state_idx < 2 or state_idx + 1 >= len(tokens):
                continue
            date_idx = next((idx for idx in range(state_idx + 2, len(tokens)) if re.match(r'^\d{2}/\d{2}/\d{4}$', tokens[idx])), -1)
            schedule = ''
            if date_idx - (state_idx + 2) >= 3:
                schedule = tokens[date_idx - 2]
            rows.append({
                'job_id': tokens[0],
    ...
    def run(self):
        ...
        scheduled_rows = [row for row in rows if row['schedule'] and row['schedule'] != '-']
        invalid_rows = [row for row in scheduled_rows if row['state'] != 'Done' or row['status_code'] not in ['0', '71']]
        metrics = {
            'job_count': len(rows),
            'scheduled_job_count': len(scheduled_rows),
            'invalid_scheduled_job_count': len(invalid_rows),
            'invalid_scheduled_jobs': invalid_rows,
        }
        thresholds = {'required_state': 'Done', 'required_status_code': '0'}
        if invalid_rows:
            return self.fail(error='스케줄이 있는 작업 중 Done/0 기준을 만족하지 않는 행이 있습니다.', metrics=metrics, thresholds=thresholds, reasons='스케줄이 있는 작업 중 Done/0 기준을 만족하지 않는 행이 있습니다.', message='스케줄 백업 상태 경고: 비정상 스케줄 작업 %s건.' % len(invalid_rows))
        return self.ok(metrics=metrics, thresholds=thresholds, reasons='스케줄이 있는 작업의 State가 Done이고 Statu 값이 0입니다.', message='스케줄 백업 상태 점검 정상')
