...
```bash
for lu in $(mpathadm list lu 2>/dev/null | awk '{print $1}'); do mpathadm show lu "$lu"; done
```
...
class Check(BaseCheck):
    ...
    def run(self):
        ...
        command = self._build_command(MPATHADM_SHOW_LU_COMMAND)
        rc, out, err = self._ssh(command)
        self._set_display_command(MPATHADM_SHOW_LU_COMMAND)
