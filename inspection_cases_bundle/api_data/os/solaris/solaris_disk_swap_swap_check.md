...
디스크 기반 swap 영역의 전체 크기, 남은 공간, 장치별 분포를 기준으로 swap 압박 여부를 점검합니다.
...
class Check(BaseCheck):
    ...
    def run(self):
        ...
        command = self._build_command(SWAP_LIST_COMMAND)
        rc, out, err = self._ssh(command)
        self._set_display_command(SWAP_LIST_COMMAND)
        ...
        if rc != 0:
            return self.fail(
                '점검 명령 실행 실패',
                message=(
                    'Solaris Disk Swap 사용률 점검에 실패했습니다. '
        ...
        if invalid_rows:
            ...
            return self.fail(
                ...
                message=(
                    'Solaris Disk Swap 사용률 점검에 실패했습니다. '
                    f'현재 상태: disk swap 장치 데이터가 비정상입니다: {invalid_summary}.'
                ),
                stdout=text,
                stderr=(err or '').strip(),
            )
        ...
        if total_blocks <= 0:
            return self.fail(
                ...
                message=(
                    'Solaris Disk Swap 사용률 점검에 실패했습니다. '
                    f'현재 상태: 전체 disk swap blocks가 {total_blocks}로 집계되어 정상적인 용량을 확인하지 못했습니다.'
                ),
                stdout=text,
                stderr=(err or '').strip(),
            )
        ...
        if used_percent > max_swap_used_percent or free_percent < min_swap_free_percent:
            return self.fail(
                'Disk Swap 사용률 임계치 초과',
                message=(
                    'Solaris Disk Swap 사용률이 기준치를 초과했습니다. '
