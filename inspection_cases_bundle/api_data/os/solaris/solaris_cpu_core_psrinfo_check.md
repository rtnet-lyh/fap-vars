...
class Check(BaseCheck):
    ...
    def _build_command(self, command):
        script = f'PATH={SOLARIS_PATH}; export PATH; {command}'
    ...
    def run(self):
        ...
        command_v = self._build_command(PSRINFO_VERBOSE_COMMAND)
        command_p = self._build_command(PSRINFO_PHYSICAL_COMMAND)

        rc_v, out_v, err_v = self._ssh(command_v)
        self._set_display_command(PSRINFO_VERBOSE_COMMAND)
        ...
        rc_p, out_p, err_p = self._ssh(command_p)
        self._set_display_command(PSRINFO_PHYSICAL_COMMAND)
        ...
        combined_output = '\n'.join(part for part in (clean_out_v, clean_out_p) if part)
        ...
        virtual_processors = self._parse_virtual_processors(clean_out_v)
        ...
        physical_processors = self._parse_physical_processors(clean_out_p)
